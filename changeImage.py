...
for image_name in files:
    ...
    if not image_name.startswith('.') and 'tiff' in image_name:
        ...
        # 1. Converting the file to RGB. The raw images from images 
        # subdirectory contains alpha transparency layers. So, it is 
        # better to first convert RGBA 4-channel format to RGB 3-channel 
        # format before processing the images. Use convert("RGB") method 
        # for converting RGBA to RGB image. 2. Then resizing it. 3. 
        # Saving it to the new path with JPEG format.
