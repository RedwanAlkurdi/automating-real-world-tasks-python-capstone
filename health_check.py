...
def check_memory_usage():
    """This function will check if Available memory is less than 
500MB"""
    # using the psutil command and converting it to dictionary and 
...
def resolve_hostname():
    """This function will check if localhost cannot be resolved to 
127.0.0.1"""
    hostname = socket.gethostbyname('localhost')
    return hostname == '127.0.0.1' 

def check_error():
    """This function will return subject according to the conditions 
met"""
    if not check_disk_usage('/'):
        ...
        return subject
    # Here we are returning none, so that we can use it later to compare 
    # it in email subject
    else:
        return None 
...
if __name__ == "__main__":
    ...
    if new_subject is not None:
        ...
        # structuring email and attaching the file. Then sending the 
        # email, using the custom module.
