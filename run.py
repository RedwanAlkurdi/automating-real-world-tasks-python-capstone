...
def catalog_data():
    """This function will return a list of dictionaries with all the 
details like name, weight, description, image_name.
    ...
    for description_name in description_files:
        # Accepting files that has txt extension and ignoring hidden 
        # files
        if not description_name.startswith('.') and 'txt' in description_name:
            ...
            # creating image name from text files and changing the 
            # extension to jpeg
            ...
            with open(description_path) as content:
                ...
                name = data[0].strip('\n')
                # extracting the second line and removing the newline 
                # and lbs. Also changing it to integer
                ...
                # removing all the none values from the list of 
                # dictionaries
                catalog = list(filter(None, catalog))
    return catalog 

def summary(input_for):
    """Generating a summary with two lists, which gives the output name 
and weight"""
    ...
    for name, weight in zip(res, we):
        ...
        # If we want to display the same list in mail. elif input_for == 
        # 'email': new_obj += name + '\n' + weight + '\n' + '\n'
    return new_obj 
